...
use App\Models\{Pedido, PedidoDetalle, OrderInven, OrderGrupo, OrderClient, Vendedor, Pago, Company, ArtDepos, PagoPedido};
use App\User;
use App\Traits\DespachosTrait;
...
class DespachosController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->permission = 'despachos';
        $this->module = 'despachos';
    }
    ...
    {
        //if ( ! hasPermission($this->permission) || Auth::user()->email!='gabydespacho@gmail.com' || Auth::user()->email!='dacabe@gmail.com' ) {
        //    abort(403);
        //}
    ...
    public function modifyQty(Request $request)
    {
        ...
        if ($detalle) {
            $qty = $request->operation == 'minus' ? $detalle->cantidad - 1 : $detalle->cantidad + 1;
    ...
    public function addToCart(Request $request) 
    {
        ...
        if (!$detalle) {
            $tasa = 45;
            $detalle = (new PedidoDetalle)->createNew($pedido->id, $request->cantidad, round($request->precio, 2), round($inven->ACTUALDL, 2), $tasa, $inven->CODIGO, $inven->DESCR, $inven->DUNIMEDD, $request->modo);
    ...
    public function saveOrder(Request $request)
    {
        ...
        $pedido = (new Pedido)->searchPendingOrder();
        if (!$pedido) {
            $pedido = (new Pedido)->createNew('', $request->descripcion, $request->rif, auth()->user()->name, $request->telefono, $request->email, 'PENDIENTE', $request->observations, $request->conditions, $request->factura, $request->porc_retencion, $request->retencion, $request->cliageret);
        } else {
