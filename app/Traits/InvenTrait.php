...
trait InvenTrait
{
    ...
    private function getJsonOrDatatableResponse(Request $request){      
        if ($request->datatable){
            $inven = (new Inven)->getData();
            return $this->getDatatableResponse($request, $inven);
        }
        
        return new InvenCollection(Inven::where('company_id', Auth::user()->company_id)->paginate($this->paginate));
    }
