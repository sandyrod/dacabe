...
<head>
    <meta charset="utf-8">
    <title>Vision 360 Vendedor</title>
    <style>
        body { font-family: DejaVu Sans, sans-serif; font-size: 11px; color: #1f2937; }
        .header { border-bottom: 2px solid #1f5f8b; margin-bottom: 12px; padding-bottom: 8px; }
        .title { font-size: 18px; font-weight: 700; color: #0d2b45; }
        ...
        .section-title { font-size: 12px; font-weight: 700; margin: 10px 0 6px; color: #0d2b45; }
...
<body>
    <div class="header">
        <div class="title">Vision 360 Vendedor</div>
        <div class="sub">
            Vendedor: {{ $vendedorSeleccionado->codigo }} - {{ $vendedorSeleccionado->nombre }} ({{ $vendedorSeleccionado->email }})
        </div>
        <div class="sub">
    ...
    <table class="kpi-grid">
        <tr>
            <td>
                <div class="kpi-label">Ventas USD</div>
                <div class="kpi-val">${{ number_format($resumen['ventas_usd'], 2, ',', '.') }}</div>
            </td>
            <td>
                <div class="kpi-label">Pedidos</div>
                <div class="kpi-val">{{ number_format($resumen['pedidos_total']) }}</div>
            </td>
            <td>
                <div class="kpi-label">Cobranza</div>
                <div class="kpi-val">${{ number_format($resumen['pagos_aprobados'], 2, ',', '.') }}</div>
            </td>
            <td>
                <div class="kpi-label">Saldo Cartera USD</div>
                <div class="kpi-val">${{ number_format($resumen['saldo_cartera_usd'], 2, ',', '.') }}</div>
            </td>
        </tr>
    </table>
