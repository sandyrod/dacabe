...
@section('titulo', config('app.name', 'Laravel') . ' - Actualización de Comisiones')

@section('titulo_header', 'Actualización Masiva de Comisiones')
...
@section('styles')
    ...
    <style>
        ...
        .comision-input.dirty+.badge-dirty-tag {
...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-3">
                <div class="stat-widget exec-card" style="border-left-color: var(--success-green);">
                    <div class="stat-label"><i class="fas fa-percentage mr-1"></i> Con Comisión</div>
                    <div class="stat-value">{{ number_format($stats['con_comision']) }}</div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="stat-widget exec-card" style="border-left-color: #b0bec5;">
                    <div class="stat-label"><i class="fas fa-ban mr-1"></i> Sin Comisión</div>
                    <div class="stat-value">{{ number_format($stats['sin_comision']) }}</div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="stat-widget exec-card" style="border-left-color: var(--accent-gold);">
                    <div class="stat-label"><i class="fas fa-chart-line mr-1"></i> Promedio Comisión</div>
                    <div class="stat-value">{{ number_format($stats['avg_comision'] ?? 0, 2) }}%</div>
                </div>
            </div>
        </div>
        ...
        <div class="exec-card p-4 mb-4">
            ...
            <hr>
            <form method="GET" action="{{ route('order-inven.modify-comisiones') }}">
                <div class="row items-center">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            <label class="text-muted small font-weight-bold uppercase">Comisión</label>
                            <select name="comision_filter" class="form-control">
                                <option value="TODOS" {{ request('comision_filter', 'TODOS') == 'TODOS' ? 'selected' : '' }}>Todos</option>
                                <option value="CON" {{ request('comision_filter') == 'CON' ? 'selected' : '' }}>Con Comisión</option>
                                <option value="SIN" {{ request('comision_filter') == 'SIN' ? 'selected' : '' }}>Sin Comisión</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-2 d-flex align-items-end">
                        <div class="form-group mb-3 d-flex w-100" style="gap: 5px;">
                            ...
                            <a href="{{ route('order-inven.modify-comisiones') }}"
        ...
        <div class="exec-card overflow-hidden mb-5">
            <div class="gradient-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0 font-weight-bold">
                    <i class="fas fa-percentage mr-2"></i> Editor de Comisiones en Lote
                </h4>
            ...
            @php
                function getComisionSortUrl($column)
                {
                ...
                function getComisionSortIcon($column)
                {
                ...
                <div class="table-responsive">
                    <table class="table exec-table table-hover mb-0">
                        <thead>
                            <tr>
                                <th width="150">
                                    <a href="{{ getComisionSortUrl('CODIGO') }}" class="text-decoration-none"
                                        style="color: inherit;">
                                        Código {!! getComisionSortIcon('CODIGO') !!}
                                    </a>
                                </th>
                                <th>
                                    <a href="{{ getComisionSortUrl('DESCR') }}" class="text-decoration-none"
                                        style="color: inherit;">
                                        Descripción {!! getComisionSortIcon('DESCR') !!}
                                    </a>
                                </th>
                                <th width="150">Departamento</th>
                                <th class="text-right" width="150">
                                    <a href="{{ getComisionSortUrl('COMISION') }}" class="text-decoration-none"
                                        style="color: inherit;">
                                        % Comisión {!! getComisionSortIcon('COMISION') !!}
                                    </a>
                                </th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    ...
                                    <td class="text-right align-middle">
                                        <input type="number" step="0.01" min="0" max="100"
                                            class="comision-input update-comision"
                                            data-codigo="{{ $product->CODIGO }}"
                                            data-original="{{ number_format($product->COMISION ?? 0, 2, '.', '') }}"
                                            value="{{ number_format($product->COMISION ?? 0, 2, '.', '') }}">
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            let dirtyChanges = {}; // key: codigo, value: {codigo, value}

            $('.update-comision').on('input change', function() {
                const input = $(this);
            ...
            $('#btn-save-all').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('order-inven.batch-update-comisiones') }}",
                    method: "POST",
                    data: {
                        _token: token,
                        updates: updates
            ...
            $('.update-comision').on('keypress', function(e) {
