...
@section('styles')
    ...
    <style>
        ...
        /* =========================================================
           ESTILOS ELEGANTES PARA DISTRIBUCIÓN DE SALDOS POR PEDIDO
           ========================================================= */
        .pedido-distribucion-item {
        ...
        .pedido-distribucion-item:hover {
        ...
        .saldo-card-check:hover .saldo-card-icon-wrap {
        ...
        /* Color schemes for checked/selected */
        .saldo-card-iva.is-selected {
            background: linear-gradient(145deg, #ffffff 0%, #fffbeb 100%) !important;
            border-color: #f59e0b !important;
            box-shadow: 0 4px 14px rgba(245, 158, 11, 0.15) !important;
        }
        .saldo-card-iva .saldo-card-icon-wrap {
            background: rgba(245, 158, 11, 0.14) !important;
            color: #d97706 !important;
        }
        .saldo-card-iva .saldo-card-title {
            color: #92400e !important;
        }
        .saldo-card-iva .saldo-card-amount {
            color: #b45309 !important;
        }

        .saldo-card-base.is-selected {
            background: linear-gradient(145deg, #ffffff 0%, #f0fdf4 100%) !important;
            border-color: #10b981 !important;
            box-shadow: 0 4px 14px rgba(16, 185, 129, 0.15) !important;
        }
        .saldo-card-base .saldo-card-icon-wrap {
            background: rgba(16, 185, 129, 0.14) !important;
            color: #059669 !important;
        }
        .saldo-card-base .saldo-card-title {
            color: #166534 !important;
        }
        .saldo-card-base .saldo-card-amount {
            color: #059669 !important;
        }

        .saldo-card-ajustes.is-selected {
            background: linear-gradient(145deg, #ffffff 0%, #eef2ff 100%) !important;
            border-color: #6366f1 !important;
            box-shadow: 0 4px 14px rgba(99, 102, 241, 0.15) !important;
        }
        .saldo-card-ajustes .saldo-card-icon-wrap {
            background: rgba(99, 102, 241, 0.14) !important;
            color: #4f46e5 !important;
        }
        .saldo-card-ajustes .saldo-card-title {
            color: #3730a3 !important;
        }
        .saldo-card-ajustes .saldo-card-amount {
            color: #4338ca !important;
        }

        /* Deseleccionado */
        .saldo-card-check.is-unselected {
        ...
        .saldo-card-check.is-unselected:hover {
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="alert alert-info mb-4">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="fw-bold fs-5">
                                    ...
                                    @endphp
                                    Monto a pagar: <b id="monto-a-pagar-alert">{{ number_format($totalConAjustes, 2, ',', '.') }}</b>
                                    <span id="moneda-simbolo-alert">{{ ($tipoPago === 'bolivares' || session('pago_cliente.tipo_pago') === 'bs') ? 'Bs.' : '$' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <form method="POST" action="{{ route('vendedores.pagos.storeMultiple') }}"
                            ...
                            @if(isset($pedidosDistribucion) && $pedidosDistribucion->count() > 0)
                            <!-- Sección: Distribución de Saldos a Abonar por Pedido -->
                            <div class="card border-0 shadow-sm mb-4" style="border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0;">
                                <div class="card-header py-3 px-4 d-flex flex-wrap justify-content-between align-items-center"
                                     style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); color: white;">
                                    <div class="d-flex align-items-center gap-2">
                                        <div style="width: 38px; height: 38px; border-radius: 10px; background: rgba(56, 189, 248, 0.15); display: flex; align-items: center; justify-content: center; border: 1px solid rgba(56, 189, 248, 0.3);">
                                            <i class="fas fa-layer-group text-info" style="font-size: 1.1rem;"></i>
                                        </div>
                                        <div>
                                    ...
                                    <div class="mt-2 mt-sm-0 d-flex gap-2">
                                        <button type="button" class="btn btn-sm btn-outline-light px-3 py-1 fw-semibold rounded-pill" id="btn-marcar-todos-saldos" style="transition: all 0.2s;">
                                            <i class="fas fa-check-square me-1"></i> Marcar Todos
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-warning px-3 py-1 fw-semibold rounded-pill" id="btn-desmarcar-todos-saldos" style="transition: all 0.2s;">
                                            <i class="far fa-square me-1"></i> Desmarcar Todos
                                        </button>
                                    </div>
                                </div>
                                <div class="card-body p-3 p-md-4" style="background: #f8fafc;">
                                    @php
                                        ...
                                        $opcionIvaActiva = session('pago_cliente.tipo_pago') === 'bs'
                                            ...
                                            : request('opcion_iva_divisa', 'retencion');
                                    @endphp
                                    @foreach($pedidosDistribucion as $ped)
                                        <div class="card mb-3 border-0 shadow-sm pedido-distribucion-item" data-pedido-id="{{ $ped->id }}"
                                             style="border-radius: 14px; background: white; border: 1px solid #e2e8f0 !important;">
                                            <div class="card-header py-2 px-3 border-bottom d-flex flex-wrap justify-content-between align-items-center"
                                                 style="background: linear-gradient(180deg, #ffffff 0%, #f8fafc 100%); border-radius: 14px 14px 0 0;">
                                                <div class="d-flex align-items-center flex-wrap gap-2">
                                                    <span class="badge px-3 py-2 fw-bold text-white shadow-sm" style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); font-size: 0.82rem; border-radius: 8px; letter-spacing: 0.3px;">
                                                        <i class="fas fa-box-open me-1"></i>Pedido #{{ $ped->id }}
                                                    </span>
                                                    <span class="badge bg-white text-secondary border px-2 py-1" style="font-size: 0.78rem; font-weight: 500;">
                                                        <i class="far fa-calendar-alt me-1 text-primary"></i>{{ $ped->fecha_formateada }}
                                                    </span>
                                                    @if(!empty($ped->factura_numero))
                                                        <span class="badge border px-2 py-1" style="background: #ecfdf5; color: #047857; border-color: #a7f3d0 !important; font-size: 0.78rem; font-weight: 600;">
                                                            <i class="fas fa-file-invoice me-1"></i>Factura: {{ $ped->factura_numero }}
                                                        </span>
                                                    @endif
                                                </div>
                                                <div class="d-flex align-items-center flex-wrap gap-2 mt-2 mt-sm-0">
                                                    <span class="badge bg-white text-dark border px-2 py-1" style="font-size: 0.78rem;">
                                                        <span class="text-muted fw-normal">Base:</span> <strong class="text-dark">$ {{ number_format($ped->saldo_base_num, 2, ',', '.') }}</strong>
                                                    </span>
                                                    @if($ped->saldo_iva_bs_num > 0.001)
                                                        <span class="badge border px-2 py-1" style="background: #fffbeb; color: #b45309; border-color: #fde68a !important; font-size: 0.78rem;">
                                                            <span class="text-muted fw-normal">IVA:</span> <strong>Bs. {{ number_format($ped->saldo_iva_bs_num, 2, ',', '.') }}</strong>
                                                        </span>
                                                    @endif
                                                    @if(abs($ped->saldo_ajustes_num) > 0.001)
                                                        <span class="badge border px-2 py-1" style="background: {{ $ped->saldo_ajustes_num >= 0 ? '#fef2f2' : '#f0fdf4' }}; color: {{ $ped->saldo_ajustes_num >= 0 ? '#b91c1c' : '#15803d' }}; border-color: {{ $ped->saldo_ajustes_num >= 0 ? '#fecaca' : '#bbf7d0' }} !important; font-size: 0.78rem;">
                                                            <span class="text-muted fw-normal">Ajuste:</span> <strong>{{ $ped->saldo_ajustes_num >= 0 ? '+' : '' }}$ {{ number_format($ped->saldo_ajustes_num, 2, ',', '.') }}</strong>
                                                        </span>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="card-body p-3">
                                                <div class="row">
                                                    ...
                                                    @if($ped->saldo_iva_bs_num > 0.001)
                                                        @php
                                                            $montoIvaDisplayBs = $opcionIvaActiva === 'retencion' && $ped->retencion_bs_num > 0
                                                                ? $ped->iva_neto_bs_num
                                                                : $ped->saldo_iva_bs_num;
                                                            $montoIvaDisplayUsd = $tasaBcvActual > 0 ? round($montoIvaDisplayBs / $tasaBcvActual, 2) : 0;
                                                        @endphp
                                                        <div class="col-12 col-md-4 mb-2 mb-md-0">
                                                            <label class="saldo-card-check saldo-card-iva is-selected">
                                                                <div class="saldo-card-left">
                                                                    <input type="checkbox" class="saldo-check-input check-saldo check-saldo-iva"
                                                                           checked
                                                                           data-pedido-id="{{ $ped->id }}"
                                                                           data-tipo="iva"
                                                                           data-monto-bs="{{ $montoIvaDisplayBs }}"
                                                                           data-monto-usd="{{ $montoIvaDisplayUsd }}">
                                                                    <div class="saldo-card-icon-wrap">
                                                                        <i class="fas fa-percentage"></i>
                                                                    </div>
                                                                    <div class="saldo-card-info">
                                                                        <span class="saldo-card-title">Saldo IVA (Bs.)</span>
                                                                        <span class="saldo-card-sub">
                                                                            {{ $opcionIvaActiva === 'retencion' && $ped->retencion_bs_num > 0 ? 'IVA Neto (Retención)' : 'IVA Total' }}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                                <div class="saldo-card-right">
                                                                    <span class="saldo-card-amount">
                                                                        Bs. {{ number_format($montoIvaDisplayBs, 2, ',', '.') }}
                                                                    </span>
                                                                    <span class="saldo-card-conv">
                                                                        ≈ $ {{ number_format($montoIvaDisplayUsd, 2, ',', '.') }}
                                                                    </span>
                                                                </div>
                                                            </label>
                                                        </div>
                                                    ...
                                                    {{-- 2. Saldo Base --}}
                                                    @if($ped->saldo_base_num > 0.001)
                                                        <div class="col-12 col-md-4 mb-2 mb-md-0">
                                                            <label class="saldo-card-check saldo-card-base is-selected">
                                                                <div class="saldo-card-left">
                                                                    <input type="checkbox" class="saldo-check-input check-saldo check-saldo-base"
                                                                           checked
                                                                           data-pedido-id="{{ $ped->id }}"
                                                                           data-tipo="base"
                                                                           data-monto-bs="{{ $ped->saldo_base_bs }}"
                                                                           data-monto-usd="{{ $ped->saldo_base_num }}">
                                                                    <div class="saldo-card-icon-wrap">
                                                                        <i class="fas fa-dollar-sign"></i>
                                                                    </div>
                                                                    <div class="saldo-card-info">
                                                                        <span class="saldo-card-title">Saldo Base ($)</span>
                                                                        <span class="saldo-card-sub">Capital del pedido</span>
                                                                    </div>
                                                                </div>
                                                                <div class="saldo-card-right">
                                                                    <span class="saldo-card-amount">
                                                                        $ {{ number_format($ped->saldo_base_num, 2, ',', '.') }}
                                                                    </span>
                                                                    <span class="saldo-card-conv">
                                                                        Bs. {{ number_format($ped->saldo_base_bs, 2, ',', '.') }}
                                                                    </span>
                                                                </div>
                                                            </label>
                                                        </div>
                                                    ...
                                                    @if(abs($ped->saldo_ajustes_num) > 0.001)
                                                        <div class="col-12 col-md-4 mb-2 mb-md-0">
                                                            <label class="saldo-card-check saldo-card-ajustes is-selected">
                                                                <div class="saldo-card-left">
                                                                    <input type="checkbox" class="saldo-check-input check-saldo check-saldo-ajustes"
                                                                           checked
                                                                           data-pedido-id="{{ $ped->id }}"
                                                                           data-tipo="ajustes"
                                                                           data-monto-bs="{{ $ped->saldo_ajustes_bs }}"
                                                                           data-monto-usd="{{ $ped->saldo_ajustes_num }}">
                                                                    <div class="saldo-card-icon-wrap">
                                                                        <i class="fas fa-sliders-h"></i>
                                                                    </div>
                                                                    <div class="saldo-card-info">
                                                                        <span class="saldo-card-title">Ajustes</span>
                                                                        <span class="saldo-card-sub">
                                                                            {{ $ped->saldo_ajustes_num > 0 ? 'Cargo adicional' : 'Descuento especial' }}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                                <div class="saldo-card-right">
                                                                    <span class="saldo-card-amount" style="color: {{ $ped->saldo_ajustes_num >= 0 ? '#4338ca' : '#059669' }};">
                                                                        {{ $ped->saldo_ajustes_num >= 0 ? '+' : '' }}$ {{ number_format($ped->saldo_ajustes_num, 2, ',', '.') }}
                                                                    </span>
                                                                    <span class="saldo-card-conv">
                                                                        {{ $ped->saldo_ajustes_bs >= 0 ? '+' : '' }}Bs. {{ number_format($ped->saldo_ajustes_bs, 2, ',', '.') }}
                                                                    </span>
                                                                </div>
                                                            </label>
                                                        </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        function actualizarMontoPendiente() {
            ...
            // Actualizar siempre el input de monto con el saldo restante según la selección de saldos y abonos previos
            const saldoRestanteSugerido = Math.max(montoPendiente, 0).toFixed(2);
            $('#monto_bs').val(saldoRestanteSugerido);
            if ($('#monto_bs_hidden').length) {
                $('#monto_bs_hidden').val(saldoRestanteSugerido);
            }
            ...
            if (montoPendiente <= 0 && totalAPagar > 0) {
                ...
                $('#btn-agregar-pago').prop('disabled', totalAPagar <= 0);
            }
        }

        // Función para recalcular los saldos seleccionados por pedido
        function recalcularDistribucionSaldos() {
            const distribucion = {};
            let sumaBs = 0;
            let sumaUsd = 0;
            let totalChecks = 0;
            ...
            $('.pedido-distribucion-item').each(function() {
                const pedidoId = $(this).data('pedido-id');
                const tieneIva = $(this).find('.check-saldo-iva').length > 0;
                const tieneBase = $(this).find('.check-saldo-base').length > 0;
                const tieneAjustes = $(this).find('.check-saldo-ajustes').length > 0;

                distribucion[pedidoId] = {
                    iva: tieneIva ? $(this).find('.check-saldo-iva').is(':checked') : false,
                    base: tieneBase ? $(this).find('.check-saldo-base').is(':checked') : false,
                    ajustes: tieneAjustes ? $(this).find('.check-saldo-ajustes').is(':checked') : false
                };
            });

            $('.check-saldo').each(function() {
                totalChecks++;
                const isChecked = $(this).is(':checked');
                const cardLabel = $(this).closest('.saldo-card-check');
                if (isChecked) {
                    cardLabel.addClass('is-selected').removeClass('is-unselected');
                    checksMarcados++;
                    sumaBs += parseFloat($(this).data('monto-bs')) || 0;
                    sumaUsd += parseFloat($(this).data('monto-usd')) || 0;
                } else {
                    cardLabel.removeClass('is-selected').addClass('is-unselected');
                }
            });

            // Guardar JSON en input oculto
            $('#distribucion-saldos-json').val(JSON.stringify(distribucion));
            ...
            const nuevoTotalFormateado = totalAPagar.toLocaleString('es-VE', formatOpt);

            if (isBolivares) {
                $('#total-bolivares').text(nuevoTotalFormateado);
            } else {
                $('#total-dolares2').text(nuevoTotalFormateado);
            }
            $('#monto-a-pagar-alert').text(nuevoTotalFormateado);
            $('#total-pagar').text(nuevoTotalFormateado + (isBolivares ? ' Bs.' : ' $'));
        ...
        $(document).ready(function() {
            ...
            // Listeners para checkboxes de distribución de saldos
            $(document).on('change', '.check-saldo', function() {
                recalcularDistribucionSaldos();
            });

            $('#btn-marcar-todos-saldos').on('click', function() {
                $('.check-saldo').prop('checked', true);
                recalcularDistribucionSaldos();
            });

            $('#btn-desmarcar-todos-saldos').on('click', function() {
                $('.check-saldo').prop('checked', false);
                recalcularDistribucionSaldos();
            });

            // Inicializar distribución de saldos y la interfaz
            recalcularDistribucionSaldos();
            actualizarListaPagos();
            actualizarTotalPagado();
            actualizarMontoPendiente();
            ...
            $('#multiplePaymentsForm').on('submit', function(e) {
                ...
                if ($('.check-saldo').length > 0 && $('.check-saldo:checked').length === 0) {
                    e.preventDefault();
                    Swal.fire('Atención', 'Debe seleccionar al menos un saldo pendiente a abonar', 'warning');
                    return false;
                }
